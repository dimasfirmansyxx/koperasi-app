...
Public Class frmrptpengambilan
    ...
    Sub getDgv()
        ...
        dgv.ReadOnly = True
        For kolom As Integer = 4 To dgv.ColumnCount - 1
            ...
            dgv.Rows(max).Cells(2).Value = 0
            dgv.Rows(max).Cells(3).Value = "Total > "
    ...
    Sub getYear()
        conn.Close()
        conn.Open()
        cmd = New MySqlCommand("SELECT * FROM tblpengambilan", conn)
    ...
    Private Sub btnviewbln_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnviewbln.Click
        ...
        Else
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("SELECT * FROM tblpengambilan WHERE bln = '" & cmbbulan.Text & "' AND thn = '" & cmbtahun.Text & "'", conn)
            reader = cmd.ExecuteReader
            reader.Read()
            If reader.HasRows Then
                conn.Close()
                conn.Open()
                adapter = New MySqlDataAdapter("SELECT * FROM tblpengambilan WHERE bln = '" & cmbbulan.Text & "' AND thn = '" & cmbtahun.Text & "' ORDER BY id DESC", conn)
                dataset = New DataSet
                adapter.Fill(dataset, "tblpengambilan")
                dgv.DataSource = dataset.Tables("tblpengambilan")
            Else
    ...
    Private Sub btnviewthn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnviewthn.Click
        ...
        Else
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("SELECT * FROM tblpengambilan WHERE thn = '" & cmbtahun.Text & "'", conn)
            reader = cmd.ExecuteReader
            reader.Read()
            If reader.HasRows Then
                conn.Close()
                conn.Open()
                adapter = New MySqlDataAdapter("SELECT * FROM tblpengambilan WHERE thn = '" & cmbtahun.Text & "' ORDER BY id DESC", conn)
                dataset = New DataSet
                adapter.Fill(dataset, "tblpengambilan")
                dgv.DataSource = dataset.Tables("tblpengambilan")
            Else
    ...
    Private Sub TextBox1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtcari.TextChanged
        ...
        Else
            ...
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("SELECT * FROM tblpengambilan WHERE id = '" & keyword & "' OR kode_anggota = '" & keyword & "' OR nama_anggota = '" & keyword & "'", conn)
            reader = cmd.ExecuteReader
            reader.Read()
            If reader.HasRows Then
                conn.Close()
                conn.Open()
                adapter = New MySqlDataAdapter("SELECT * FROM tblpengambilan WHERE id = '" & keyword & "' OR kode_anggota = '" & keyword & "' OR nama_anggota = '" & keyword & "'", conn)
                dataset = New DataSet
