...
Public Class frmrptsimpanan
    ...
    Sub getYear()
        conn.Close()
        conn.Open()
        cmd = New MySqlCommand("SELECT * FROM tblsimpanan", conn)
    ...
    Private Sub btnviewbln_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnviewbln.Click
        ...
        Else
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("SELECT * FROM tblsimpanan WHERE bln = '" & cmbbulan.Text & "' AND thn = '" & cmbtahun.Text & "'", conn)
            reader = cmd.ExecuteReader
            reader.Read()
            If reader.HasRows Then
                conn.Close()
                conn.Open()
                adapter = New MySqlDataAdapter("SELECT * FROM tblsimpanan WHERE bln = '" & cmbbulan.Text & "' AND thn = '" & cmbtahun.Text & "' ORDER BY id DESC", conn)
                dataset = New DataSet
                adapter.Fill(dataset, "tblsimpanan")
                dgv.DataSource = dataset.Tables("tblsimpanan")
            Else
    ...
    Private Sub btnviewthn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnviewthn.Click
        ...
        Else
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("SELECT * FROM tblsimpanan WHERE thn = '" & cmbtahun.Text & "'", conn)
            reader = cmd.ExecuteReader
            reader.Read()
            If reader.HasRows Then
                conn.Close()
    ...
    Private Sub TextBox1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtcari.TextChanged
        ...
        Else
            ...
            conn.Close()
            conn.Open()
            cmd = New MySqlCommand("SELECT * FROM tblsimpanan WHERE id = '" & keyword & "' OR kode_anggota = '" & keyword & "' OR nama_anggota = '" & keyword & "'", conn)
            reader = cmd.ExecuteReader
            reader.Read()
            If reader.HasRows Then
                conn.Close()
                conn.Open()
                adapter = New MySqlDataAdapter("SELECT * FROM tblsimpanan WHERE id = '" & keyword & "' OR kode_anggota = '" & keyword & "' OR nama_anggota = '" & keyword & "'", conn)
                dataset = New DataSet
                adapter.Fill(dataset, "tblsimpanan")
                dgv.DataSource = dataset.Tables("tblsimpanan")
            Else
